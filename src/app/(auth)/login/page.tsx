...
//           employee: '/employee',
//         };
//         router.push(dashboardRoutes[userData.role] || '/employee');
//       } catch (error) {
...
//       toast.success(`Welcome back, ${user.full_name || user.name || user.email}! 🎉`, {
//         duration: 3000,
//         icon: '👋',
//       });
...
//       router.push(dashboardRoutes[user.role] || '/employee');
...
//                       className={`w-full pl-12 pr-4 py-3 border-2 rounded-xl focus:outline-none transition-colors bg-white/50 ${
...
export default function LoginPage() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // ✅ FIXED: Call login with correct parameters
      ...
      // ✅ FIXED: Response structure (no .data property)
      const { access_token, user } = response;

      // Store token and user info (already done in authAPI.login, but double-check)
      localStorage.setItem('access_token', access_token);
      localStorage.setItem('user', JSON.stringify(user));
            ...
            <div className="relative z-10">
              ...
              <form onSubmit={handleLogin} className="space-y-6">
                ...
                >
                  ...
                  <div className="space-y-1 text-xs text-gray-600">
                    <p>Admin: admin@test.com / Admin123</p>
                    <p>User: hetpatelsk@gmail.com / your_password</p>
                  </div>
                </motion.div>
              </form>
            </div>
